...
void main() async {
  ...
  if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
    try {
      ...
      const window_manager.WindowOptions windowOptions = window_manager.WindowOptions(
        size: Size(400, 400),
        center: true,
        alwaysOnTop: true,
      ...
      await window_manager.WindowManager.instance.waitUntilReadyToShow(windowOptions, () async {
...
class SettingsStartScreen extends StatelessWidget {
  ...
  void _selectColor(BuildContext context, bool isWorkColor, PomodoroSettings settings) {
    showDialog(
      context: context,
          ...
          children: [
            ...
            ...List.generate(
              ...
              (index) => TextField(
                ...
                onChanged: (value) {
                  final intValue = int.tryParse(value) ?? settings.sectionMinutes[index];
            ...
            ListTile(
              ...
              trailing: Container(width: 30, height: 30, color: settings.workColor),
              onTap: () => _selectColor(context, true, settings),
            ),
            ListTile(
              title: const Text('Rest Color'),
              trailing: Container(width: 30, height: 30, color: settings.restColor),
...
class _PomodoroScreenState extends State<PomodoroScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    timeLeftInSeconds = widget.settings.sectionMinutes.reduce((a, b) => a + b) * 60;
  ...
  Future<void> _loadAudio() async {
    try {
      ...
      await _player.setClip(end: Duration(milliseconds: (widget.settings.audioDuration * 1000).toInt()));
  ...
  void _applyDesktopSettings() async {
    if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
      try {
        await window_manager.WindowManager.instance.setOpacity(widget.settings.opacity);
        await window_manager.WindowManager.instance.setSize(Size(widget.settings.windowSize, widget.settings.windowSize));
        await window_manager.WindowManager.instance.setMovable(widget.settings.isDraggable);
        await window_manager.WindowManager.instance.setIgnoreMouseEvents(widget.settings.isClickThroughEnabled);
      } catch (e) {
  ...
  void _checkSectionTransition() {
    final totalTime = widget.settings.sectionMinutes.reduce((a, b) => a + b) * 60;
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
                ...
                (index) => TextField(
                  ...
                  onChanged: (value) {
                    localSettings.updateSectionMinutes(
                        index, int.tryParse(value) ?? localSettings.sectionMinutes[index]);
                  },
                ),
              ),
              if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) ...[
                CheckboxListTile(
                  ...
                  onChanged: (value) {
                    ...
                    window_manager.WindowManager.instance.setIgnoreMouseEvents(value ?? false);
                  },
                ),
                CheckboxListTile(
                  title: const Text('Draggable'),
                  ...
                  onChanged: (value) {
                    ...
                    window_manager.WindowManager.instance.setMovable(value ?? false);
                  },
                ),
                const Text('Window Opacity'),
                Slider(
                ...
                Slider(
                  ...
                  onChanged: (value) {
                    ...
                    window_manager.WindowManager.instance.setSize(Size(value, value));
                  },
                ),
              ],
            ],
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              widget.settings.sectionMinutes = localSettings.sectionMinutes.toList();
              widget.settings.updateClickThrough(localSettings.isClickThroughEnabled);
        ...
        children: [
          MouseRegion(
            ...
            child: GestureDetector(
              onTap: widget.settings.isClickThroughEnabled ? null : _showSettingsDialog,
              child: Center(
                child: CustomPaint(
                  size: const Size(300, 300),
                  painter: CircleTimerPainter(
                    timeLeftInSeconds: timeLeftInSeconds,
                    totalTimeInSeconds: widget.settings.sectionMinutes.reduce((a, b) => a + b) * 60,
          ...
          Positioned(
            ...
            left: 20,
            child: IgnorePointer(
              ignoring: widget.settings.isClickThroughEnabled && !_isBackButtonHovered,
              child: AnimatedOpacity(
                ...
                child: MouseRegion(
                  ...
                  onExit: (_) {
                    ...
                    if (!kIsWeb && (Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
                      window_manager.WindowManager.instance.setOpacity(widget.settings.opacity);
                    }
                  },
                  child: IconButton(
...
class CircleTimerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < sections.length; i++) {
      ...
      final sectionSweepDegrees = (sectionDurationSeconds / totalTimeInSeconds) * 360;
      final solidColor = i % 2 == 0 ? workColor : restColor;
      final lightColor = i % 2 == 0 ? workColor.withOpacity(0.3) : restColor.withOpacity(0.3);
      ...
      if (secondsElapsedSoFar >= cumulativeSecondsPassed + sectionDurationSeconds) {
        ...
        final elapsedInCurrentSection = secondsElapsedSoFar - cumulativeSecondsPassed;
        final elapsedPartSweepDegrees = (elapsedInCurrentSection / sectionDurationSeconds) * sectionSweepDegrees;
        ...
        final remainingPartSweepDegrees = sectionSweepDegrees - elapsedPartSweepDegrees;
    ...
    final elapsedAngleDegrees = 360 * (1 - timeLeftInSeconds / totalTimeInSeconds);
    ...
    final textPainter = TextPainter(
      text: TextSpan(
        text: '${(timeLeftInSeconds ~/ 60).toString().padLeft(2, '0')}:${(timeLeftInSeconds % 60).toString().padLeft(2, '0')}',
